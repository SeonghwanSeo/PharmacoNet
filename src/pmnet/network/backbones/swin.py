...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        Return:
            x: Output feature, tensor size (B, D, H, W, C).
        """
        B, D, H, W, C = x.shape
        shortcut = x
        x = self.norm1(x)
        ...
        pad_w0 = pad_h0 = pad_d0 = 0
        pad_w1 = (self.window_size - H % self.window_size) % self.window_size
        pad_h1 = (self.window_size - W % self.window_size) % self.window_size
        ...
        )  # nW*B, window_size, window_size, window_size, C (nW: num_windows)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        Return:
            x: Input feature, tensor size (B, D/2, H/2, W/2, 2*C).
        """
        B, D, H, W, C = x.shape
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, int, int, int, Tensor, int, int, int]:
        ...
        if self.downsample is not None:
            ...
            Wd, Wh, Ww = (D + 1) // 2, (H + 1) // 2, (W + 1) // 2
            return x, D, H, W, x_down, Wd, Wh, Ww
        else:
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.patch_size: tuple[int, int, int] = to_3tuple(patch_size)
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            _, _, Wd, Wh, Ww = x.shape
            x = x.flatten(2).transpose(1, 2)  # B, Wd*Wh*Ww, C'
            x = self.norm(x)
            x = x.transpose(1, 2).view(B, self.embed_dim, Wd, Wh, Ww)  # B, C', Wd, Wh, Ww
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        num_heads: Sequence[int] = [3, 6, 12, 12],
        window_size: int = 6,
        mlp_ratio: float = 4.0,
        qkv_bias: bool = True,
        qk_scale: float | None = None,
        ...
        out_indices: int | Sequence[int] = [0, 1, 2, 3],
    ):
        super().__init__()
        ...
        if self.ape:
            patches_resolution = [
                imgsz // patchsz for imgsz, patchsz in zip(to_3tuple(image_size), to_3tuple(patch_size), strict=True)
            ]
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, embed_dim, *patches_resolution))  # (1, C, D, H, W)
    ...
    def forward(self, x: Tensor) -> Sequence[Tensor]:
        """Forward function."""
        x = self.patch_embed(x)  # B, C, D, H, W -> B, C', Wd, Wh, Ww

        B, _, Wd, Wh, Ww = x.shape
        if self.ape:
            # interpolate the position embedding to the corresponding size
            absolute_pos_embed = F.interpolate(self.absolute_pos_embed, size=(Wd, Wh, Ww), mode="trilinear")
            x = (x + absolute_pos_embed).flatten(2).transpose(1, 2)  # B, Wd*Wh*Ww, C'
        else:
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
        x = x.view(B, Wd, Wh, Ww, -1)  # B, Wd, Wh, Ww, C'

        outs = []
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, _, _, _, x, _, _, _ = layer(x)  # B, Wd, Wh, Ww, C' | B, Wd', Wh', Ww', C''
