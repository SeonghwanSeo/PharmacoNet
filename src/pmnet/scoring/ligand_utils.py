...
class PharmacophoreNode:
    def __init__(
        self,
        atom_indices: int | tuple[int, ...],
        center_indices: None | int | tuple[int, ...] = None,
    ):
        if center_indices is None:
            center_indices = atom_indices
        self.atom_indices: int | tuple[int, ...] = atom_indices
        self.center_indices: int | tuple[int, ...] = center_indices
...
def get_pharmacophore_nodes(
    pbmol: pybel.Molecule,
) -> dict[str, list[PharmacophoreNode]]:
    obmol = pbmol.OBMol
    ...
    pos_charged = [
        PharmacophoreNode(idx)
        for idx, obatom in enumerate(obatoms)
        if is_quartamine_N(obatom) or is_tertamine_N(obatom) or is_sulfonium_S(obatom)
    ]
    neg_charged = []
