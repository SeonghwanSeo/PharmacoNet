...
def scoring_matching_pair(
    ...
    for cluster_node_match1, cluster_node_match2 in itertools.product(
        cluster_node_match_list1, cluster_node_match_list2
    ):
        ...
        np.sum(
            np.abs(distance_sigma_array) < DISTANCE_SIGMA_THRESHOLD,
            axis=0,
            out=num_pass,
        )
        ...
        np.dot(
            weights / stds.reshape(-1),
            np.exp(-0.5 * distance_sigma_array**2),
            out=likelihood,
        )
